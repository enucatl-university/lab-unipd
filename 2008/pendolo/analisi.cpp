...
// 	cout.setf(ios::fixed,ios::floatfield);
	...
	cout << "La semidispersione \u00e8 " << (max - min)/2. << endl; 
	int conta=0;
//scarta valori oltre  media-3sigma e media+3sigma
	...
	double *cattivi = new double[scartati];
	int k=0;
	int c=0;
	for(int i=0;i<osservazioni;i++)
	{
...
cout << "\u00c8 stato scartato un solo valore (" << cattivi[0] << ")" << endl;
	else
	{cout << "Sono stati scartati " << scartati << " valori: " << endl;
	for(int i=0; i< scartati; i++)
	cout << cattivi[i] << endl;}
	med = media(buoni,conta);
	s = sigma(buoni,conta);
	min = trovamin(buoni,conta);
	max = trovamax(buoni,conta);
	...
	cout << "La nuova semidispersione \u00e8 " << (max - min)/2. << endl;
}	
else if (scartati == 0)
cout << "Non \u00e8 stato scartato nessun valore." << endl;
else return 0;
	}
...
double sigma(double valori[], int l)
{
	...
	double si = sqrt(somma/double(l-1));
	return si;
}
