...
import sys, math, time, Gnuplot, Gnuplot.PlotItems, Gnuplot.funcutils

try:
	import rpy
except:
	print 'impossibile caricare rpy'
...
class DatiSperimentali(list):
		...
		def calcresidui(self, other):
				a = self.interpolazionelineare(other)
				self.residui = []
				for i in range(len(self.valori)):
					self.residui.append(a[2] + a[0]*self.valori[i] - other.valori[i])
				return self.residui

		def calcerrpost(self, other):
				self.calcresidui(other)
				sumsq = 0.
				n = len(self.residui)
				for residuo in self.residui:
					sumsq += residuo**2
...
#					sum += self.valori[i]
#				for i in range(n):
#					sumsq += self.valori[i]**2
#				self.correl = -sum/math.sqrt(n*sumsq)
#				return self.correl
