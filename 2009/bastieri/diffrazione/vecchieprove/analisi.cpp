...
// 	cout << integers[i] << " " << sines[i] << endl;
	...
	for( int i = 1 ; i <= lines ; i++ )	hDiff.Fill( step[i], -intensity[i] );
	TApplication app( "App", &argc, argv );

	TSpectrum s(100);
// 	float *source = (float*)intensity;
// 	float *dest = new float[lines];
	Int_t nfound = s.Search( &hDiff, 10, "", 0.08 );
	Float_t *xpeaks = s.GetPositionX();

	for( int i = 0; i < nfound; i++){
	cout << "picco trovato in : " << xpeaks[i] << endl;
	}
...
// 	hDiff1.Draw();
	...
	TF1 *line1 = new TF1("line1","pol1",1,5);
	TF1 *line2 = new TF1("line2","pol1",-5,-1);
