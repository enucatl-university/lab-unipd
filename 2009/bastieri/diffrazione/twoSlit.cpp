...
class TwoSlit: public Slit {
// methods for analyzing minima in diffraction pattern, one or more slits.
	public:
	TwoSlit( Int_t found, Float_t* peaks );
	TGraphErrors* drawAndFit();
	Double_t* getSlitWidth();
	private:
	static const int nslits = 2;
};



TwoSlit::TwoSlit( Int_t found, Float_t* peaks ) : Slit( found-5, peaks ){
}

// TwoSlit::~TwoSlit( ){
// }

TGraphErrors* TwoSlit::drawAndFit(){
